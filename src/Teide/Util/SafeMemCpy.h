...
template <class Dest, class Source>
    requires(
        std::ranges::contiguous_range<Dest> && std::ranges::sized_range<Dest> &&
        // !std::ranges::constant_range<Dest> && // C++23
        std::ranges::contiguous_range<Source> && std::ranges::sized_range<Source>
        && !(
            sizeof(std::ranges::range_value_t<Dest>) % sizeof(std::ranges::range_value_t<Source>) != 0
            && sizeof(std::ranges::range_value_t<Source>) % sizeof(std::ranges::range_value_t<Dest>) != 0))
