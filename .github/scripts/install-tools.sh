...
function install_from_pip {
    package=$1
    echo "Installing package \"${package}\" from pip..."
    /c/hostedtoolcache/windows/Python/3.14.6/x64/Scripts/pip3 install ${package} || panic "Failed to install package from pip"
}
...
exec_dirs=$(find ${installed_dir} \
        -type f -executable -exec dirname '{}' \; |
sort | uniq)
